...
-- v07 12Kbyte read
-- v08 32K read for including soundroms ( WillFA7S )
--
-- v2.0
-- each rom 64KByte (128 sectors ) expected
-- crc check for first 32KB
-- expected CRC at address 0xFFFE & 0xFFFF
...
use IEEE.numeric_std.all;
	...
	entity SD_Card is
		...
		-- crc check
		calc_checksum	: buffer  std_logic_vector(15 downto 0); -- calculated checksum
		read_checksum	: buffer  std_logic_vector(15 downto 0); -- read checksum
   ...
   architecture Behavioral of SD_Card is		
		type STATE_T is ( Startdelay, send_read_request, wait_for_read, continue, 
								...
								check_sector_byte, inc_addr_and_unset_wr, stop_read, delay_and_repeat, check_crc, all_done, error, stop );
...
SD_CARD_READ: entity work.SPI_Master --read i byte by byte (slooow)
    ...
    port map (
		...
		SD_Card: process (i_Clk, i_Rst_L )
			...
			variable sectorcount: integer range 0 to 50000;
				   ...
				   case cmd_count is
					   when 1 => TX_Data_A <= x"FF" & x"FFFFFFFFFFFF" & x"FFFFFFFFFFFFFF"; -- init 
						...
						when 7 => TX_Data_A <= x"FF" & CMD18 & x"FFFFFFFFFFFFFF";	
									 do_not_disable_SS <= '1';
									 -- special: calculate sector on SD
									 -- where to read rom depending on dip switch
									 -- first rom starts at sector 660
									 -- we have 64KByte of data
									 -- which is 128 sectors 512Byte each
									 gamenumber := to_integer(unsigned(selection));									 
									 sectorcount := gamenumber * 128 + 660;									 
				...
				when continue =>		
					if (TX_Done_A = '0') then -- Master sets back TX_Done when ready again				
								...
								case cmd_count is
									...
									when 8 => -- we send CMD12 to stop read sector, all sectors red, now check crc
										wr_rom <= '0';		
										state_A <= check_crc;	
		...
		--------------------------------------
				...
				when inc_addr_and_unset_wr =>		
								...
								-- crc data reached?
								if unsigned(address_sd_card) = x"FFFE" then
									read_checksum(15 downto 8) <= data_sd_card;
								end if;
								
								-- finished?																
								if unsigned(address_sd_card) = x"FFFF" then 		-- 64K 
				...
				when check_crc =>		
					if ( calc_checksum /= read_checksum ) then
						crc_error <= '1';
						state_A <= error;
					else
						state_A <= all_done;
...
CRC16_rom: entity work.crc16_ccitt
		port map (
			...
			crc_en => wr_rom and not address_sd_card(15), -- crc only for first 32K
			rst => i_Rst_L,
			clk => i_clk,
			crc_out => calc_checksum
      );		
